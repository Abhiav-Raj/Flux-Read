...
export const createExercise = async (req, res) => {
  try {
    ...
    console.log("Received contents:", contents); // Debugging

    if (!Array.isArray(contents)) {
      return res.status(400).json({ message: "Invalid contents. Expected an array." });
    }

    const newExercise = new Exercise({ name, description, difficulty });

    const exerciseContents = await Promise.all(contents.map(async (contentData) => {
      const { contentType, text, imageUrl, description, mcqs } = contentData;
      
      const newContent = new Content({ contentType, text, imageUrl, description });
      
      const contentMCQs = await Promise.all(mcqs.map(async (mcqData) => {
        const newMCQ = new MCQ(mcqData);
        return await newMCQ.save();
      }));

      newContent.mcqs = contentMCQs.map(mcq => mcq._id);
