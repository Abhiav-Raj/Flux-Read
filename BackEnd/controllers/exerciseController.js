...
export const getExerciseById = async (req, res) => {
  ...
  export const getExercisesByAge = async (req, res) => {
    try {
        ...
        // Find exercises where difficulty.minAge <= age <= difficulty.maxAge
        const exercises = await Exercise.find({ 
            'difficulty.minAge': { $lte: age },
            'difficulty.maxAge': { $gte: age }
        })
            .populate({
                path: 'content',
                populate: {
        ...
        if (!exercises.length) {
            return res.status(404).json({ message: `No exercises found for age ${age}` });
        }
