...
const usersalldata = async () => {
  try {
    ...
    const response = await axios.get(
      "http://localhost:8080/user/getAllDetails", // ✅ Correct API endpoint
      { headers: { Authorization: `Bearer ${token}` } }
    );

    setUserDetails(response.data);
    localStorage.setItem("userDetails", JSON.stringify(response.data)); // ✅ Persist user data
  } catch (error) {
...
useEffect(() => {
  const checkAuth = async () => {
    ...
    await usersalldata(); // Call only when token is present
  };
  ...
  const calculateAverageSpeed = (results) => {
    ...
    return validSpeeds.length > 0 ? (totalSpeed / validSpeeds.length).toFixed(2) : 0;
  };
